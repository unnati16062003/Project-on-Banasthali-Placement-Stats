...
menu_data = [
    ...
    {'icon': "	fas fa-user-circle",'label':"Admin"},
...
<body>
	...
	<div id="slider">
        <div class="abc">
         <img src="http://localhost:8501/media/a203660564f8b8b0acf4477fc34e5d14e3e01af0b2af9c3a701547eb.png" />
...
<div class="cont2">
                <div class="adi">
                    ...
                    <button class="b3">APPLY NOW</button>
...
'''
    ...
    if username not in usernames and authentication_status != None:
        ...
        #authentication_status=True
            ...
            logged_menu=st.sidebar.radio('Select an option',('Add reviews','View reviews','Check Sentiment','Prediction'))
            if logged_menu=="Add reviews":
            ...
            if logged_menu=="View reviews":
                collection5=db["Review"]
                alldocs=collection5.find()
                for s in alldocs:
                    statement=s["Name"]
                    statement1=str(s["Year"])
                    statement2=s["Btech Branch"]
                    statement3=s['Review']
                    st.subheader("Review by "+statement)
                    st.subheader("Year :"+statement1)
                    st.subheader("Btech Branch \:"+statement2)
                    st.subheader(" Student Review \:"+statement3)
                #st.session_state.il=banasthalite
 if menu_id == "Register":
    page_bg_img = '''
<style>
...
'''
    ...
    #st.text(authentication_status)
        ...
        #authentication_status=True
                    ...
                    st.subheader(f'Welcome *{name}*')
                    authenticator.logout("Logout","sidebar")
                    ad_menu=st.sidebar.radio('Select an option',('Upload Placement Data','Delete Placement Data','Upload Student Data','Delete Student Data'))
                    if ad_menu=="Upload Placement Data":
                        collection3=db["PlacementStats"]
                    ...
                    if ad_menu=="Delete Placement Data":
                        ...
                        yr = delete_form.text_input("Enter the year whose placement data is to be deleted")
                        ...
                        if dell:
                            ...
                            if yrin not in yrs:
                                ...
                                if up.deleted_count >0:
                                    ...
                                    st.success(st1)
                    if ad_menu=="Upload Student Data":
                        uploaded_file = st.file_uploader("Choose a file")
                        if uploaded_file is not None:
                            dataframe = pd.read_excel(uploaded_file)
                            st.table(dataframe)
                            ...
                            st.success("Student data is uploaded successfully")
                    if ad_menu=="Delete Student Data":
                        collection4=db["BanasthaliID"]
                        alldata=collection4.find()
                        yrs=[]
                        for item in alldata:
                            if item["Year"] not in yrs:
                                yrs.append(item["Year"])
                        #yr = st.text_input('Enter the year whose placement data is to be deleted')
                        delete_form1= st.form(key='delete_form1', clear_on_submit=True)
                        yr = delete_form1.text_input("Enter the year for which students'data has to be deleted")
                        #dell=st.button("Submit")
                        dell = delete_form1.form_submit_button(label='Delete records')
                        if dell:
                            yrin=int(yr)
                            if yrin not in yrs:
                                st.warning("Records of no such year are present!!")
                                ...
                                collection4=db["BanasthaliID"]
                                rec={"Year":yrin}
                                up=collection4.delete_many(rec)
                                if up.deleted_count >0:
                                    st1="Student data for the "+yr+" year students is deleted successfully"
                                    st.success(st1)
