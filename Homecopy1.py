...
'''
    ...
    #df=pd.read_excel("E:\data.xlsx")
        ...
        if user_menu=='Year-wise visualization':
            ...
            bar2=ax2.bar(y.index-bar_width/2,y['CTC offered(LPA)'],bar_width,color='purple')
            plt.ylabel('CTC offered(LPA)')
            plt.xlabel('Year of placement')
            st.title('Highest packages offered yearwise, in '+str(len(w['Year of placement'].unique()))+" years")
            st.pyplot(fig2)
...
background-attachment: local;
    ...
    #df=pd.read_excel("E:\data.xlsx")
            ...
            with st.sidebar:
                img = Image.open("grp.png")
                st.image(img, width=200)
            user_menu=st.sidebar.radio('Select an option',('Overall data preview','Overall analysis','Year-wise analysis','Branch-wise visualization','Add reviews','View Reviews and Public sentiment','Predict Placement Stats'))
            if user_menu == 'Overall data preview':
                years,company,branch,CTC_range = helper.company_year_branch_ctc(df)
                selected_year = st.sidebar.selectbox("Select Year",years)
                selected_company = st.sidebar.selectbox("Select Company name", company)
